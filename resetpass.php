...
&& ($_GET["action"]=="reset") && !isset($_POST["action"])){
  ...
  "SELECT * FROM `resetpass` WHERE `key`='".$key."' and `username`='".$email."';"
  );
...
$pass1 = md5($pass1);
mysqli_query($con,
"UPDATE `user` SET `password`='".$pass1."', `trn_date`='".$curDate."' 
WHERE `username`='".$email."';"
);

mysqli_query($con,"DELETE FROM `resetpass` WHERE `username`='".$email."';");
